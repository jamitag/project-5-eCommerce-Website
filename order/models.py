...
class Cart(models.Model):
    ...
    item = models.ForeignKey(Product, on_delete=models.CASCADE) # Item to be put in cart
    quantity = models.IntegerField(default=1) # Number of items to be ordered
    purchased = models.BooleanField(default=False) # If cart items are purchased, itmes will be removed from cart
...
class Order(models.Model):
    ...
    created = models.DateTimeField(auto_now_add=True)
    paymentId = models.CharField(max_length=264, blank=True, null=True) # Assigns an order a transaction id
